...
{
	BallVisionT last_ball=pVision->Ball(pVision->Cycle()-4);
...
void ContactChecker::judgeOne()
{
	if (true){
		...
		for (std::vector<PlayerVisionT>::iterator it = AllPlayer.begin(); it != AllPlayer.end(); it++) {
			...
			if (it->Valid()){
				if (number>6){color=1;}
				if (_ball.Pos().dist(it->Pos()) <=(Param::Vehicle::V2::PLAYER_SIZE + Param::Field::BALL_SIZE+judge_one[color])) {
					_isJudged = true;
...
void ContactChecker::refereeJudge(const CVisionModule* pVision)
{
	...
	if (pVision->GetCurrentRefereeMsg()!="gameStop"&&pVision->GetCurrentRefereeMsg()!="gameHalt"&&pVision->Ball().Valid()){
