...
bool CWorldModel::IsBallKicked(int num)
{
	...
	if(KickStatus::Instance()->isForceClosed()){
		GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(0, 100), "isForceClosed", COLOR_BLACK);
		return false;
	}
	bool isballkicktemp = BallStatus::Instance()->IsBallKickedOutNew(_pVision, num);
	//if (isballkicktemp) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(0, 100), "ballkick", COLOR_BLACK);
	//else GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(0, 100), "nokick", COLOR_BLACK);
	return isballkicktemp;
}

double CWorldModel::timeToTarget(int player, const CGeoPoint target) {
    return predictedTimeWithRawVel(_pVision->OurPlayer(player), target, CVector(0, 0));
}
...
bool CWorldModel::ball2MarkingTouch(CGeoPoint leftUp, CGeoPoint rightDown)
{
	...
	int markTouchNum =  DefenceInfo::Instance()->getMarkingTouchNum(leftUp,rightDown);
	if (markTouchNum!=0){
		dist = vision()->Ball().Pos().dist(vision()->OurPlayer(markTouchNum).Pos());
		if (dist <100){
			result = true;
		}
	}
...
bool CWorldModel::isMarkingFrontValid(CGeoPoint checkPos, double checkDir)
{
	...
	if (vision()->Ball().X() < vision()->OurPlayer(toMeNum).X() && vision()->OurPlayer(toMeNum).X()<0){
		//cout<<"out"<<endl;
		return false;
	}
...
const bool CWorldModel::canPassOnBallPos(int current_cycle,CGeoPoint& passPos,CGeoPoint& guisePos,int myNum ){
	...
	if (last_cycle < current_cycle) {
		...
		CGeoPoint passPosOne,passPosTwo,passPosThree,PassPosFour;

		CGeoPoint predictBall=BallSpeedModel::Instance()->posForTime(60,_pVision);
		NormalPlayUtils::generatePassPoint(predictBall,passPosOne,passPosTwo, passPosThree, PassPosFour);
